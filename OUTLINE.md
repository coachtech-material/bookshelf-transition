...
>
> 階層: 3層（Part > Chapter > Section）/ 全 4 部・10 章・32 節。
> 種類凡例: **概念** = 理論・仕組み / **ハンズオン** = 既習概念の実践（冒頭に概念 Section への逆リンク）/ **混合** = 概念＋即実践（実践見出しはまとめの直前）。
> **章末ハンズオン（4-3・8-3）と総合ハンズオン（Part 4）は完全に独立**（積み上げない）。例外として、2-3 で作るサンドボックスは Part 2 の混合 Section（3-2・5-1）の Tinker 実践と、7-3（Postman 疎通確認）で再利用する。詳細は末尾「ハンズオン設計」。
...
→ ゴール: これまで未経験の REST API を、ルート設計から API Resource・検索/ページネーション・JSON 例外まで一通り設計・実装でき、API 開発ツール（Postman）で動作を確認できる。API は専用スターターキットでハンズオンする。

### Chapter 7: API の基礎と Resource（3 Section）

**ゴール**: REST API のルートを設計し、API Resource でレスポンスを整形し、Postman で API に疎通確認できる。
...
- **7-3 API 開発ツール（Postman）**
  - 種類: 混合
  - ゴール: API テストツールの必要性を理解し、Postman をインストールして GET / POST 等のリクエスト（`Accept: application/json` ヘッダー・JSON ボディ）を送り、コレクションと環境変数（`{{base_url}}`）でリクエストを整理できる。2-3 のサンドボックスに使い捨ての疎通用ルートを足して Postman で確認する。
  - 前提: [7-1, 2-3]
  - 参考資料: [Postman 公式](https://www.postman.com/) / 旧教材 tutorial-11（API 開発環境の準備・疎通確認）
  - 意図: 概念 Section で設計・整形した API を「実際に叩く」手段を補う。8-3 と総合ハンズオンの動作確認はこの Postman を使う。サンドボックス（2-3）を再利用し、疎通用ルートで確認して後片付けする（積み上げない）。
...
- **8-3 ハンズオン: 公開タスク API を実装する**
  - 種類: ハンズオン（逆リンク: 7-1, 7-2, 7-3, 8-1, 8-2）/ **独立**
  - ゴール: スターターキット `laravel-api-starter`（Task・User のモデル/マイグレーション/シーダーまでが土台。**コントローラと API ルートは未実装**・認証なし）をクローンし、公開 REST API（一覧=検索/ページネーション付き・詳細・登録・更新・削除）を実装する。`Api\V1` 名前空間のコントローラと API 用 FormRequest、API Resource、JSON 例外、`per_page` の上限クランプを含める。動作確認は Postman（7-3 で導入）で行い、`curl` を併記する。
  - 前提: [7-1, 7-2, 7-3, 8-1, 8-2]
...
- **9-3 マイグレーションとモデル（リレーション定義）**
  - 種類: ハンズオン
  - ゴール: users/categories/tasks/tags/task_tag のマイグレーションを作成し、**FK 制約と ON DELETE CASCADE**（タスク削除で task_tag も自動削除）を設定する。モデルに 1対多・多対多リレーションと `$fillable`/`casts` を定義する。`migrate:fresh --seed` 用のシーダー/ファクトリも用意。
  - 前提: [3-1, 9-2]
...
- **9-5 CRUD（カテゴリ・タスク）と多対多操作**
  - 種類: ハンズオン
  - ゴール: カテゴリとタスクの CRUD をリソースコントローラ・FormRequest 込みで実装する。多対多操作として、タスクへのタグ付けを `sync`（純粋ピボット）で実装する。FormRequest では **DB 連動バリデーション**（更新時の `Rule::unique()->ignore()`・`exists`・配列要素 `tags.*`）を扱い、カテゴリ削除時は **関連タスクがあれば削除を拒否**する削除ガード（日本語フラッシュ文言）も実装する。
  - 前提: [3-2, 9-4]

### Chapter 10: 総合ハンズオン② — 認可・集計・API・テスト・仕上げ（6 Section）

**ゴール**: CRUD に認可・集計・公開 API・自動テストを積み上げ、完成度の高い状態に仕上げる。教材全体を締め括る。
...
- **10-2 集計機能**
  - 種類: ハンズオン
  - ゴール: よく使われているタグのランキング（`Tag::withCount('tasks')` ＝多対多の集計）と、詳細画面のカテゴリ別タスク数（`loadCount`）等の集計機能を、N+1 を回避して実装する。
  - 前提: [5-1, 5-2, 9-5]
...
- **10-5 仕上げ（Pint・README・Issue 駆動の振り返り・最終チェック）**
  - 種類: ハンズオン
  - ゴール: Laravel Pint で整形、README（環境構築手順・ER 図・API 一覧・使用技術）を整備、Issue 駆動/PR の流れを振り返り、最終チェックリストで仕上げる。
  - 前提: [10-4]
...
- **API 開発ツール（7-3）**: 専用キットは使わず、**2-3 のサンドボックスに使い捨ての疎通用ルート（`/api/ping`・`/api/echo`）を足して Postman で確認**（混合）。確認後にルートを削除して元に戻す（積み上げない）。
...
- User / Category（1対多: Category→Task）/ Task（belongsTo User・Category）/ Tag（多対多: Task↔Tag via `task_tag` ＝純粋ピボット）
- 多対多は純粋ピボット `task_tag`（タグ付け）を実装する。3-1・3-2 で扱う複合 unique ピボット＋`toggle` は概念とサンドボックスで学び、総合ハンズオンでは純粋ピボット 1 つに絞る（新模擬案件の「お気に入り」相当を成果物に作り込まないため。詳細は末尾「※ 題材スコープの方針」）。FK は ON DELETE CASCADE。
- 認証（Fortify）・タスク/カテゴリ CRUD・タグ付け（`sync`）・所有者認可（Policy）・カテゴリ削除ガード・集計（よく使われているタグのランキング `Tag::withCount('tasks')`／カテゴリ別タスク数 `withCount`・`loadCount`）・公開 API（`Api\V1`・Resource・検索・ページネーション・JSON 例外）・テスト（カバレッジ 60%）

**※ 題材スコープの方針（2026-06-15 改訂）**: 本教材が埋めるのは **2026年2月以前の教材 → 2026年3月以降の教材** の差分（公開 API・認可・自動テスト・多対多実践・集計・Sail という汎用概念）であり、新模擬案件 Bookshelf の機能セットを写すことではない。`docs/diff-analysis.md` は当初「Basic = 新教材範囲」と置き Bookshelf を新教材の代理として差分を取ったが、その設計は Bookshelf 固有の作り（お気に入りトグル・複合 unique ピボット・お気に入り数ランキング等）を成果物に持ち込みやすい。そこで総合ハンズオンの題材からは **お気に入り機能を外し**、多対多は純粋ピボット（タグ付け）に絞り、集計は **タグ利用数ランキング**（多対多の `withCount`）＋カテゴリ別タスク数（`loadCount`）へ付け替えた。`toggle`・複合 unique ピボットは汎用スキルとして 3-1・3-2（概念＋サンドボックス）に残す。
