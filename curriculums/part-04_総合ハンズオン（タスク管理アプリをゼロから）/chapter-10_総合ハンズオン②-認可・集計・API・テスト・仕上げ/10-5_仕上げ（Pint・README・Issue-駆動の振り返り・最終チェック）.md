...
- Issue と Pull Request を使った開発の流れを振り返り、最終チェックリストでアプリを確認する

このセクションでは、作ったアプリを、きちんと動く完成した状態に仕上げます。動くだけでなく、他人が読める・再現できる形に整えます。

---

## 導入: 動くだけでは終わりではない

アプリは動くようになりました。しかし、成果物として見たときには、もう一段の仕上げが要ります。コードの書式がそろっているか、README を読めば誰でも環境を再現できるか、どんな流れで開発したかが伝わるか。ここを整えておくと、レビューする人にとっても、後から自分が読み返すときにも、ぐっと扱いやすくなります。

このセクションでは、Pint で書式を整え、README を書き、開発の流れ（Issue・Pull Request）を振り返り、最終チェックリストで全体を確認します。
...
タスクをカテゴリで分類し、タグ付け・ランキング・公開 API を備えた Laravel アプリケーションです。
...
💡 すでにローカルで完成させたこのアプリも、`git init` でリポジトリにし、GitHub に push しておくと、公開やレビューに使えます（Git の基本操作は既習です）。

### 🏃 Step 4: 最終チェックリストで確認する

最後に、アプリが「クローンした人の環境でも、記載どおりに動く」ことを確認します。次を順に確かめてください。

- [ ] `git clone` → `composer install` → `sail up -d` → `migrate:fresh --seed` → `npm run dev` の手順で、ゼロから起動できる
- [ ] `migrate:fresh --seed` が成功し、初期データ（ユーザー・カテゴリ・タグ・タスク）が入る
- [ ] 登録・ログイン・ログアウトができる
- [ ] カテゴリ・タスクの CRUD が動き、タグ付け（`sync`）が動く
- [ ] 他人のタスクは編集・削除できない（403）
- [ ] ランキングとカテゴリ別タスク数が表示される
...
- 最終チェックリストで、「ゼロから再現できる」「テストが通る」「書式が整っている」を確認した
